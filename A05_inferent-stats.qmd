# Introduction to inferential statistics {#sec-introduction-to-inferential-statistics}
...
The standard deviation of a sampling distribution of $\bar{x}$ values is called the **standard error of the mean** or simply **standard error**[^a05_inferent-stats-1]. The **standard error** provides us a metric to assess the level of precision of our sample estimate, the smaller the standard error the more likely that estimate is an accurate representation of the true population.

[^a05_inferent-stats-1]: You can of course, calculate the standard error for any sample statistic but we frequently just call it the standard error and assume that from the context it is clear which statistic we are referring to

Typically, we would calculate the standard error by dividing the standard deviation by the square root of the number of sample data[^a05_inferent-stats-2].

[^a05_inferent-stats-2]: Deriving this equation which allows you to determine the standard error without resampling is beyond the scope of our class so we'll just go straight to the solution.
...
[^a05_inferent-stats-3]: We will look at the specific examples of testing whether means and frequencies are statistically different.
...
-   `generate()`: generate a null distribution[^a05_inferent-stats-4].
...
[^a05_inferent-stats-4]: data reflecting the null hypothesis

Which statistical test you apply will depend on the type of data you are looking at[^a05_inferent-stats-5] and whether they fulfill specific assumptions[^a05_inferent-stats-6]. We are going to learn how to perform a t-test to evaluate the difference between two means and a $\chi^{2}$ test of independence to evaluate the difference between frequencies.

[^a05_inferent-stats-5]: For example, are we looking at continuous or numerical data.

[^a05_inferent-stats-6]: For example, we might assume that the data has a normal distribution.
...
At first, stating our hypothesis as "there is no difference" seems a bit odd because that's not normally what we are interested in. However, it is actually quite difficult to prove an idea but much easier to **disprove** and idea. Therefore, we want to formulate a hypothesis that we can falsify. If the sample provides enough evidence against the null hypothesis then we can **reject** the hypothesis, otherwise we **fail to reject** the null hypothesis[^a05_inferent-stats-8].

[^a05_inferent-stats-8]: Be careful not to use phrases like "we proved" or we "accept" the null hypothesis.

::: {.callout-tip title="Give it a try"}
You can use a standard way of formulating your null hypothesis by identifying your **dependent** and **independent variable** of your experiment[^a05_inferent-stats-9]. For the null hypothesis you will always state that the **Independent variable** does not affect your **dependent variable**.
...
[^a05_inferent-stats-9]: These are also referred to as the **response** and **explanatory** variables.
...
That gives us our observed test-statistic. However, we want to know how this value compares to the sampling distribution of the test statistic based on data that is generated according to the null hypothesis[^a05_inferent-stats-10].

[^a05_inferent-stats-10]: We would refer to this as the null distribution.
...
-   In your own words, describe what standard error is and what it tells us about a data set and how it should inform how you interpret your results.
...
observed_statistic <- Adelie %>%
  specify(response = flipper_length_mm,  # specify response/dependent
          explanatory = sex) %>%         # specify explanatory/independ
  calculate(stat = "diff in means",      # test statistic
            order = c("male", "female")) # order to subtract the mean values
...
observed_statistic <- df %>%
  specify(response = bill_depth_mm,         # specify response/dependent
          explanatory = species) %>%  # specify explanatory/independ
  calculate(stat = "diff in means",      # test statistic
...
null_dist <- df %>%
  specify(response = bill_depth_mm,
          explanatory = species) %>%        # specify values we are interested in
  hypothesize(null = "independence") %>%       # null hypothesis: sex and weight are independent
  generate(reps = 1000,                        # generate 1000 samples for null distribution
           type = "permute") %>%               # randomly assign sex to weight to break association
  calculate(stat = "diff in means",            # statistic is t
